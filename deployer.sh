...
export OCP_SONARQUBE_ROUTE_EDGE_TLS=${OCP_SONARQUBE_ROUTE_EDGE_TLS:="true"}
...
if [ "$INTERACTIVE" = "true" ]; then
	...
	read -rp "Secure TLS Edge for SonarQube Route? ($OCP_SONARQUBE_ROUTE_EDGE_TLS): " choice;
	if [ "$choice" != "" ] ; then
		export OCP_SONARQUBE_ROUTE_EDGE_TLS="$choice";
	fi
...
if [ "$OCP_CREATE_PROJECT" == "true" ]; then
    oc $OC_ARG_OPTIONS new-project $OCP_PROJECT_NAME --description="CI/CD Static Code Testing with SonarQube" --display-name="CI/CD - SonarQube"
fi
...
if [ "$PERSIST_DATA" == "true" ]; then
    if [ "$OCP_SONARQUBE_ROUTE_EDGE_TLS" ==  "true" ]; then
        oc $OC_ARG_OPTIONS process -f sonarqube-persistent-template-secure.yml \
        ...
        -p SONAR_LDAP_CONTEXTFACTORY="$SONAR_LDAP_CONTEXTFACTORY" | oc $OC_ARG_OPTIONS apply -f-
    else
        oc $OC_ARG_OPTIONS process -f sonarqube-persistent-template.yml \
        -p POSTGRES_PERSISTENT_VOLUME_CLAIM_SIZE="$POSTGRES_PERSISTENT_VOLUME_CLAIM_SIZE" \
        -p POSTGRES_CONTAINER_MEMORY_SIZE_LIMIT="$POSTGRES_CONTAINER_MEMORY_SIZE_LIMIT" \
        -p POSTGRES_CONTAINER_CPU_LIMIT="$POSTGRES_CONTAINER_CPU_LIMIT" \
        -p SONARQUBE_MEMORY_LIMIT="$SONARQUBE_MEMORY_LIMIT" \
    ...
    if [ "$OCP_SONARQUBE_ROUTE_EDGE_TLS" ==  "true" ]; then
        oc $OC_ARG_OPTIONS process -f sonarqube-template-secure.yml \
        -p POSTGRES_CONTAINER_MEMORY_SIZE_LIMIT="$POSTGRES_CONTAINER_MEMORY_SIZE_LIMIT" \
        -p POSTGRES_CONTAINER_CPU_LIMIT="$POSTGRES_CONTAINER_CPU_LIMIT" \
        -p SONARQUBE_MEMORY_LIMIT="$SONARQUBE_MEMORY_LIMIT" \
        -p SONARQUBE_CPU_LIMIT="$SONARQUBE_CPU_LIMIT" \
        ...
        oc $OC_ARG_OPTIONS process -f sonarqube-template.yml \
        -p POSTGRES_CONTAINER_MEMORY_SIZE_LIMIT="$POSTGRES_CONTAINER_MEMORY_SIZE_LIMIT" \
        -p POSTGRES_CONTAINER_CPU_LIMIT="$POSTGRES_CONTAINER_CPU_LIMIT" \
        -p SONARQUBE_MEMORY_LIMIT="$SONARQUBE_MEMORY_LIMIT" \
        -p SONARQUBE_CPU_LIMIT="$SONARQUBE_CPU_LIMIT" \
